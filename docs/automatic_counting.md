...
For current exercise recognition, use [`analyze --engine adaptive`](user_manual.md#train-your-personal-model).
This page documents the preserved generic `count` command and its v2 regression
baseline. Its historical examples are development checks, not fresh blind results.
...
The report now includes `pauses.csv`; the plot marks joined rests in amber. Frozen v1 results remain in the original validation folders. The v2 result (local artifact: `data/processed/exercise-04-pauses/counts.json`), plot (local artifact: `data/processed/exercise-04-pauses/automatic_count.png`), comparison checks (local artifact: `data/processed/exercise-04-pauses/validation.json`) and provisional reference (local artifact: `data/processed/exercise-04-pauses/reference.json`) are saved separately.
...
Automatic boundaries and complete motion cycles (local artifact: `data/processed/exercise-02-auto/automatic_count.png`)
...
Source CSV hashes, settings, chosen axes, internal reference window, every cycle boundary and fit measurements are saved in counts.json (local artifact: `data/processed/exercise-02-auto/counts.json`). Original source files were hash-checked after the runs and remain unchanged.
