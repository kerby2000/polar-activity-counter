...
**Historical legacy-engine update.** The current adaptive model treats the fourth
pull-up attempt separately and has since passed a fresh mixed blind test. See
[Adaptive analysis](adaptive_analysis.md) and [Examples](examples.md). The results
below preserve the earlier model's behaviour and the physical-recording evidence.
